...
class NotificationPreferenceDialog:
    ...
    def submit(self) -> None:
        ...
        if not selected_name:
            messagebox.showwarning("No Selection", "Please select a notification preference.")
            return
        
        # Find the enum value from the selection
        for pref in NotificationPreference:
            if pref.name == selected_name:
                self.selected_preference = pref
                break
...
async def get_notification_preference(parent_window: tk.Tk) -> Optional[NotificationPreference]:
    ...
    Returns:
        NotificationPreference if selected, None if cancelled
    """
...
async def get_notification_preference_simple(parent_window: tk.Tk) -> Optional[NotificationPreference]:
    """
    Simple notification preference collection without API key requirements.
    ...
    Note: This is the preferred method now that voice notifications use local Chatterbox TTS.
    """
    return await get_notification_preference(parent_window) 